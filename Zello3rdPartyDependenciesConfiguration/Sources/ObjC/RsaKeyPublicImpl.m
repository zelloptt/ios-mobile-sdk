...
- (NSData *)encrypt:(NSData *)data {
  ...
  NSInteger res = RSA_public_encrypt((int32_t)data.length, [data bytes], [encData mutableBytes], self.rsaKey, RSA_PKCS1_PADDING);
  if (res <= 0) {
    return nil;
  }
...
- (BOOL)verify:(NSData *)data withOffset:(NSInteger)offset andLength:(NSInteger)length andSignature:(NSString *)signature {
...
- (BOOL)verify:(unsigned char *)md5 withSignature:(NSString *)signature {
  ...
  if (!md5 || !self.rsaKey || sigLength < 4) {
    ...
    NSData *signatureEncoded = [[NSData alloc] initWithBytes:[signature cStringUsingEncoding:NSUTF8StringEncoding] length:sigLength + 1];
...
- (void)reset {
  self.serialized = nil;
  if (self.rsaKey && !self.keyPassedIn) {
