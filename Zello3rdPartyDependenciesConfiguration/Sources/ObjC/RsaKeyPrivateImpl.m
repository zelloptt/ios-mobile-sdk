...
- (NSString *)sign:(NSData *)data {
    ...
    unsigned int len;
    if (!RSA_sign(NID_md5, md, sizeof md, (unsigned char *)[sign mutableBytes], &len, self.privatekey) ||
        len > sign.length) {
        return nil;
    }
...
- (NSData *)decrypt:(NSData *)data withOaep:(BOOL)oaep {
    ...
    int res = RSA_private_decrypt((int32_t)data.length, ptrFrom, ptrTo, self.privatekey, padding);
    if (res == -1) {
        padding = oaep ? RSA_PKCS1_PADDING : RSA_PKCS1_OAEP_PADDING;
        res = RSA_private_decrypt((int32_t)data.length, ptrFrom, ptrTo, self.privatekey, padding);
    }
    if (res <= 0) {
        return nil;
    }
...
- (BOOL)isNullOrEmpty:(NSString *)target {
    return (NSNull *)target == [NSNull null] || target == nil ||
