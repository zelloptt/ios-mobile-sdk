...
@interface ZAsyncSocketImpl () <GCDAsyncSocketDelegate> {
...
- (BOOL)connectTo:(NetworkAddress *)addr error:(NSError *__autoreleasing *)errPtr {
  ...
  [self performBlock: ^{
    self->_connectTag = [self->_syncho nextTag];
  }];
  ...
  if (res) {
    ...
    [self performBlock: ^{
      [self->_syncho releaseTag:self->_connectTag];
    }];
  }
  return _connected;
}
...
- (void)socket:(GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port {
  ZLogInfo(@"%@ didConnectToHost using %@ %@", _remoteAddr, sock.isIPv6 ? @"IPv6" : @"IPv4", _remoteAddr.tls ? @"TLS" : @"");
  if (_voip) {
    [sock performBlock:^{
      if (@available(iOS 16.0, *)) {
        // No VoIP socket support on iOS 16
        // Zello will crash if we enable a VoIP Socket when built with XCode 14
      } else {
        BOOL result = [sock enableBackgroundingOnSocket];
        ZLogDebug(@"%@ enabled VoIP: %@", self->_remoteAddr, result ? @"Success" : @"Error");
      }
  ...
  if ([_delegate respondsToSelector:@selector(onSocket:didConnectToHost:port:)]) {
    [_delegate onSocket:self didConnectToHost:host port:port];
  }
}
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag {
  ...
  if ([_delegate respondsToSelector:@selector(onSocket:didReadData:withTag:)]) {
    [_delegate onSocket:self didReadData:data withTag:tag];
  } else {
...
- (NSTimeInterval)socket:(GCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
  ...
  if ([_delegate respondsToSelector:@selector(onSocket:shouldTimeoutReadWithTag:elapsed:bytesDone:)]) {
    return [_delegate onSocket:self shouldTimeoutReadWithTag:tag elapsed:elapsed bytesDone:length];
  }
  return 0;
}
...
- (NSTimeInterval)socket:(GCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
  ...
  if ([_delegate respondsToSelector:@selector(onSocket:shouldTimeoutWriteWithTag:elapsed:bytesDone:)]) {
    return [_delegate onSocket:self shouldTimeoutWriteWithTag:tag elapsed:elapsed bytesDone:length];
  }
  return 0;
}
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err {
  ...
  if ([_delegate respondsToSelector:@selector(onSocketDidDisconnect:)]) {
    [_delegate onSocketDidDisconnect:self];
  }
}
